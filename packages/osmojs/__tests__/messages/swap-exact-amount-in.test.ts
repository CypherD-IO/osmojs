...
it("AminoConverter.MsgSwapExactAmountIn", async () => {
  ...
  const back =
    AminoConverter["/osmosis.gamm.v1beta1.MsgSwapExactAmountIn"].fromAmino(msg);

  expect(back).toMatchSnapshot();

  const final =
    AminoConverter["/osmosis.gamm.v1beta1.MsgSwapExactAmountIn"].toAmino(back);
